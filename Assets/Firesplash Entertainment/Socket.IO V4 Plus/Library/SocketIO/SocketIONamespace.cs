...
namespace Firesplash.GameDevAssets.SocketIOPlus
{
    ...
    public class SocketIONamespace
    {
        ...
        public void Disconnect()
        {
            ...
            FireEvent("disconnect", CreateMessagePayload("io client disconnect"));
            shallConnect = false;
        ...
        internal void Disconnect(bool serverInitiated, string message)
        {
            ...
            if (state != ConnectionState.DISCONNECTED) FireEvent("disconnect", CreateMessagePayload(message)); //disconnect could propagate twice under some circumstances in native mode because we're using multiple independent threads
            state = ConnectionState.DISCONNECTED;
        }
        ...
        internal void Submit(SocketIOPacket packet)
        {
            ...
            switch (packet.type)
            {
                ...
                case PacketType.CONNECT:
                    ...
                    {
                        //TODO log Error
                        FireEvent("connect_error", CreateMessagePayload("Received connect packet without sid payload. This is a server-side protocol violation."));
                        state = ConnectionState.DISCONNECTED;
                        return;
                    }
        ...
        /// <summary>
        /// Used, to locally create a payload structure identical to the one we get from remote errors
        /// </summary>
        /// <param name="message"></param>
        /// <returns></returns>
        object[] CreateMessagePayload(string message)
        {
            JObject messageObj = new JObject();
            messageObj.Add("message", message);
            return new object[] { messageObj };
        }
